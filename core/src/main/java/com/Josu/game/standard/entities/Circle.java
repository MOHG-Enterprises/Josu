...
public class Circle {
    ...
    // New fields for judgment display;
    private Texture judgmentTexture = null;
    ...
    private static final float JUDGMENT_DURATION = 0.5f;
     ...
     * @param screenX               the x-coordinate (in screen space) for the
     *                              circle
     * @param screenY               the y-coordinate (in screen space) for the
     *                              circle
     * @param color                 a float array representing RGBA values for
     *                              tinting the overlay
    ...
    public void render(SpriteBatch batch) {
        ...
        if (judgmentTexture != null) {
            float judgmentX = position.x + (texture.getWidth() - judgmentTexture.getWidth()) / 2;
            float judgmentY = position.y + (texture.getHeight() - judgmentTexture.getHeight()) / 2;
            batch.draw(judgmentTexture, judgmentX, judgmentY);
            return;
        }
    ...
    /**
     * Updates the approach circle's scale and marks the circle inactive if the user
     * misses.
     * 
    ...
    public void update(float delta) {
        ...
        // If a judgment is set, count down its timer.
        if (judgmentTexture != null) {
            judgmentTimer -= delta;
            if (judgmentTimer <= 0) {
                active = false; // After displaying the judgment, mark the circle inactive.
            }
            return; // Skip normal approach updates while showing judgment.
        }
     ...
     * @return true if the point is inside the circle and the circle is active,
     *         false otherwise
     */
    ...
    /**
     * Plays the hit sound at a reduced volume, resets the approach circle, and
     * deactivates the circle.
     */
    public boolean wasHit() {
        return hit;
    }

    public void hit(Texture judgmentTexture) {
        hitSound.play(0.2f);
        resetApproachCircle();
        hit = true;
        ...
        if (judgmentTexture != null) {
            ...
            judgmentTimer = JUDGMENT_DURATION;
        } else {
            active = false;
        }
    }
